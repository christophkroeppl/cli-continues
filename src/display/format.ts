 ...
 */
export const sourceColors = Object.fromEntries(
...
export function formatSessionColored(session: UnifiedSession): string {
  ...
  const repoDisplay =
    session.repo || session.cwd.split("/").slice(-2).join("/") || "";
  ...
  const summary = (
    session.summary
      ? chalk.white(summaryText.slice(0, 44))
      : chalk.gray(summaryText)
  ).padEnd(44);
