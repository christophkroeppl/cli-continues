...
async function findSessionDirs(): Promise<string[]> {
  return listSubdirectories(COPILOT_SESSIONS_DIR).filter((dir) =>
...
export async function parseCopilotSessions(): Promise<UnifiedSession[]> {
  ...
  for (const sessionDir of dirs) {
    try {
      ...
      const stats = fs.existsSync(eventsPath)
        ? await getFileStats(eventsPath)
        : { lines: 0, bytes: 0 };
      ...
      sessions.push({
        ...
        lines: stats.lines,
        bytes: stats.bytes,
        createdAt: safeDate(workspace.created_at, new Date()),
        updatedAt: safeDate(workspace.updated_at, new Date()),
...
export async function extractCopilotContext(
  ...
  for (const event of events.slice(-resolvedConfig.recentMessages * 2)) {
    if (event.type === "user.message") {
      const content =
        event.data?.content || event.data?.transformedContent || "";
      if (content) {
        recentMessages.push({
          role: "user",
          content,
  ...
  const { summaries: toolSummaries, filesModified } =
    extractCopilotToolSummaries(events, resolvedConfig);
...
function extractCopilotToolSummaries(
  ...
  const toolCounts = new Map<
    string,
    {
      ...
      samples: Array<{
        summary: string;
        data?: import("../types/index.js").StructuredToolSample;
      }>;
    }
  >();
  ...
  const summaries = Array.from(toolCounts.entries()).map(
    ([name, { count, samples }]) => ({
      name,
      count,
      samples,
...
function buildCopilotSampleData(
  ...
  switch (category) {
    ...
    case "ask":
      return {
        category: "ask",
        question: ((args.question as string) || "").slice(0, 80),
      };
    default:
      return {
        category: "mcp",
