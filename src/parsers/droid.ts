...
function readSettings(jsonlPath: string): DroidSettings | null {
  ...
  try {
    if (fs.existsSync(settingsPath)) {
      ...
      logger.debug(
        "droid: settings validation failed",
        settingsPath,
        ...
        for (const block of event.message.content) {
          if (block.type === "text" && block.text) {
            if (
              ...
              !block.text.startsWith("/") &&
              !block.text.includes("Session Handoff")
            ) {
...
export async function parseDroidSessions(): Promise<UnifiedSession[]> {
  ...
  for (const filePath of files) {
    try {
      const { sessionStart, firstUserMessage, firstTimestamp, lastTimestamp } =
      ...
      const createdAt = firstTimestamp
        ? safeDate(firstTimestamp, fileStats.birthtime)
        : fileStats.birthtime;
      const updatedAt = lastTimestamp
        ? safeDate(lastTimestamp, fileStats.mtime)
        : fileStats.mtime;
...
function extractPendingTasks(events: DroidEvent[]): string[] {
  ...
  const todosText =
    typeof lastTodo.todos === "string"
      ? lastTodo.todos
      : lastTodo.todos?.todos || "";
