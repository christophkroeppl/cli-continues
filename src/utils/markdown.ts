...
export function generateHandoffMarkdown(
  ...
  if (
    sessionNotes?.tokenUsage &&
    (sessionNotes.tokenUsage.input > 0 || sessionNotes.tokenUsage.output > 0)
  ) {
    lines.push(
  ...
  if (
    ...
    (sessionNotes.cacheTokens.read > 0 || sessionNotes.cacheTokens.creation > 0)
  ) {
    lines.push(
  ...
  if (sessionNotes?.thinkingTokens && sessionNotes.thinkingTokens > 0) {
    lines.push(
      `| **Thinking Tokens** | ${sessionNotes.thinkingTokens.toLocaleString()} |`
    );
  }
  ...
  // ── Subagent Results ──
  if (
    sessionNotes?.subagentResults &&
    sessionNotes.subagentResults.length > 0
  ) {
  ...
  if (sessionNotes?.reasoning && sessionNotes.reasoning.length > 0) {
    ...
    lines.push("");
    for (const thought of sessionNotes.reasoning.slice(
      0,
      config.thinking.maxHighlights
    )) {
      lines.push(`- ${thought}`);
    }
    lines.push("");
  ...
  if (session.originalPath) {
    ...
    lines.push("");
    lines.push(
      `This session was extracted from **${
        labels[session.source] || session.source
      }** session data.`
    );
    lines.push(`- **Session file**: \`${safePath(session.originalPath)}\``);
...
function renderWriteSection(
  ...
  for (const sample of tool.samples.slice(caps.writeEditDetailed)) {
    ...
    if (d && d.category === "write") {
      const stats = d.diffStats
        ? ` (+${d.diffStats.added} -${d.diffStats.removed})`
        : "";
...
function renderEditSample(sample: ToolSample, maxDiffLines: number): string[] {
  ...
  const statsTag = d.diffStats
    ? ` (+${d.diffStats.added} -${d.diffStats.removed} lines)`
    : "";
...
function renderGrepSection(
  ...
  for (const sample of shown) {
    ...
    if (d && d.category === "grep") {
      ...
      const count =
        d.matchCount !== undefined ? ` — ${d.matchCount} matches` : "";
...
function renderGlobSection(
  ...
  for (const sample of shown) {
    ...
    if (d && d.category === "glob") {
      const count =
        d.resultCount !== undefined ? ` — ${d.resultCount} files` : "";
...
function renderCompactSection(
  ...
  const cap = ["search", "fetch"].includes(category)
    ? caps.grepGlobSearchFetch
    : caps.mcpTaskAsk;
...
function formatCompactSample(sample: ToolSample, category: string): string {
  ...
  switch (d.category) {
    case "search": {
      const countStr =
        d.resultCount !== undefined ? ` — ${d.resultCount} results` : "";
      const preview = d.resultPreview
        ? ` "${d.resultPreview.slice(0, 60)}..."`
        : "";
...
function renderSubagentResults(
  ...
  for (const r of results) {
    ...
    if (r.status === "completed" && r.result) {
      ...
      const text =
        r.result.length > maxChars
          ? r.result.slice(0, maxChars) + "\u2026"
          : r.result;
...
function renderReasoningChain(steps: ReasoningStep[]): string[] {
  ...
  for (const step of steps) {
    const label = `**${capitalize(step.purpose)}** (step ${step.stepNumber}/${
      step.totalSteps
    })`;
    const thought =
